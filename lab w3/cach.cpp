...
int accessLevel(int blockID, int level) {
    if (level == numLevels)
    {
        ...
        lastServedBy = "Main Memory";
        ++mainMemoryHits;
        return mainMemoryTime;
   }
    if (contains(cacheContents[level], blockID))
    {
        hitsPerLevel[level]++;
        lastServedBy = "l" + level + 1;
        return hitTimeArr[level];  
    }

    int lowcost = accessLevel(blockID, level + 1);

    lowcost += hitTimeArr[level];

    if (cacheContents[level].size() < capacity_[level])
    {
        cacheContents[level].push_back(blockID);
    }
